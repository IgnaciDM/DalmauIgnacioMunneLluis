...
public class LlistaAccessosTest {
    ...
    void testConstructor() {
        ...
        assertEquals(4, llistaAccessos.getLlistaAccessos().length);
    }

    @Test
    void testAfegirAcces() throws ExcepcioCamping {
        int initialSize = llistaAccessos.getLlistaAccessos().length;
        Acces newAcces = new CamiTerra("CT2", true, 50.0f);
        llistaAccessos.afegirAcces(newAcces);
        assertEquals(initialSize + 1, llistaAccessos.getLlistaAccessos().length);
    }

    @Test
    void testBuidar() {
        llistaAccessos.buidar();
        assertEquals(0, llistaAccessos.getLlistaAccessos().length);
    }
    ...
    void testCalculaAccessosAccessibles() throws ExcepcioCamping {
        ...
        assertEquals(4, accessibleCount); // All are accessible in setup

        // Add a non-accessible path
        Acces nonAccessible = new CamiTerra("CT3", false, 50.0f);
        llistaAccessos.afegirAcces(nonAccessible);
        assertEquals(4, llistaAccessos.calculaAccessosAccessibles());
    }
    ...
    void testGetLlistaAccessos() {
        Acces[] accessos = llistaAccessos.getLlistaAccessos();
        assertEquals(4, accessos.length);
        assertTrue(accessos[0] instanceof Acces);
    }
